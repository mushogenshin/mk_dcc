...
#  3. Otherwise if you want to set the checker size of shells, click the
#  'Set Checker size of shells' button

#  If 'Face shells' option is selected it will scale the UVs of a shell of faces as a whole, for all the shells
#  If 'UV Shells' option selected it will scale the UVs of a UV shell as a
#  whole, for all the UV shells
...
def get_sel_faces_UV_ratio(set_active):
    ...
    if set_active == 1:
        ...
        if (len(orig_sele) > 3):
            cmds.text(current_obj, edit=True,
                      l='Objects : ' + 'Multiple objects')
        elif (len(orig_sele) == 1):
            cmds.text(current_obj, edit=True, l='Object : ' + orig_sele[0])
        elif (len(orig_sele) == 2):
            cmds.text(current_obj, edit=True, l='Objects : ' +
                      orig_sele[0] + ', ' + orig_sele[1])
        elif (len(orig_sele) == 3):
            cmds.text(current_obj, edit=True, l='Objects : ' +
                      orig_sele[0] + ', ' + orig_sele[1] + ', ' + orig_sele[2])
        mel.eval("select -cl")
...
def set_UV_ratio(obj):
    ...
    for i in range(0, len(orig_sele)):
        ...
        current_ratio = sys.modules[__name__].get_sel_faces_UV_ratio(0)
        if current_ratio == 0:
            current_ratio = 1
        ...
        cmds.polyEditUV(pu=u_pivot, pv=v_pivot,
                        su=scale_factor, sv=scale_factor)
...
def collect_shells_and_set_shells_UV_ratio():
    ...
    while (len(main_list) > 0):
        mel.eval("select " + main_list[0])
        mel.eval("SelectUVShell")
        prog = ((1.000 * orig_len - 1.000 * len(main_list)) /
                (1.000 * orig_len)) * 20
        cmds.progressBar(progressControl, edit=True, pr=prog)
...
def show_window():
    ...
    UV_window = cmds.window(title="Maya Checker Size Tool",
                            iconName='Short Name', rtf=1, s=1, widthHeight=(300, 100), mxb=0)
    ...
    cmds.columnLayout(adjustableColumn=True)
    UV_get_but = cmds.button(label='Pick Checker Size', en=True,
                             annotation='Store the checker size', command='uvSizeChecker.get_sel_faces_UV_ratio(1)')
    current_obj = cmds.text(w=175, l='', al='center')
    cmds.setParent('..')
    cmds.setParent('..')
    cmds.frameLayout(label="Set Size",  w=30, h=110,
                     mh=5, mw=5, bs="etchedOut")
    cmds.columnLayout(adjustableColumn=True)
    UV_set_but = cmds.button(label='Set Checker Size of object(s)', en=True,
                             annotation='Change checker size of one or more meshes', command='uvSizeChecker.set_UV_ratio(1)')
    UV_shell_but = cmds.button(label='Set Checker Size of shells (Single object)', en=True,
                               annotation='Change checker size of object with several shells', command='uvSizeChecker.set_shell_button()')
...
def shell_UV_scaler():
    ...
    try:
        ...
        mel.eval("polyTransfer -uv 1 -ch off -ao \"" +
                 combined_mesh[0] + "\" \"" + sele[0] + "\"")
...
#--------------------------------------------------------------
# modified for ILM Tools only

def get_sel_faces_UV_ratio_ilm(set_active):
    global active_ratio
    totalUVArea = 0
    totalFaceArea = 0
    orig_sele = cmds.ls(sl=True, fl=True)
